...
%
% cond1 and cond2: the event types to be compared (c orresponding to the
...
elseif isa(MCPA_pattern,'float')
    ...
    warning('MCPA_pattern is class float. Proceeding with floating point matrix data for backwards compatibility. Parameters must be entered manually.');
    subj_mat = MCPA_pattern;
    
    incl_subjects = input('Please enter the vector of subjects to include (ex: [1 2 5]) or hit ENTER to include all: ');
    if isempty(incl_subjects), incl_subjects = 1:size(subj_mat,3); end
    ...
    error('ERROR: MCPA_pattern is not recognized as class struct or float.');
    return;
...
if nargin < 4
    ...
    if setsize > size(subj_mat,2)
        warning('Setsize is larger than the total number of channels. Running analysis on all %d channels simultaneously', setsize);
        setsize = size(subj_mat,2);
    end
    
    warning('Setsize not specified. Running analysis on all %d channels simultaneously', setsize);
