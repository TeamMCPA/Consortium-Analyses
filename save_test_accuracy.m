function results_struct = save_test_accuracy(results_struct, predicted_labels, correct_labels, set_idx, s_idx)
...
if size(predicted_labels,2) > 1 % test labels will be a column vector if we don't do pairwise
    ...
    for comp = 1:size(correct_labels,1)
        results_struct.accuracy_matrix_test(correct_labels(comp,1),correct_labels(comp,2),set_idx,s_idx) = subj_acc(comp);
    end
    ...
    for cond_idx = 1:n_cond
        ...
        results_struct.test_accuracy(cond_idx).subsetXsubj(:,s_idx) = cond_acc;
        results_struct.test_accuracy(cond_idx).subjXfeature(s_idx,:) = cond_acc;
