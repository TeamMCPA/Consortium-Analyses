function results_struct = save_accuracy(results_struct, correct_labels, predicted_labels, set_idx, s_idx)
...
if size(predicted_labels,2) > 1 % test labels will be a column vector if we don't do pairwise
    if s_idx==1 && set_idx == 1, results_struct.accuracy_matrix = nan(length(results_struct.conditions),length(results_struct.conditions),min(size(results_struct.subsets,1),results_struct.max_sets),length(results_struct.incl_subjects)); end
    ...
    for comp = 1:size(correct_labels,1)
        if size(correct_labels,2)==1
            results_struct.accuracy_matrix(correct_labels(comp,1),:,set_idx,s_idx) = subj_acc(comp);
        else
            results_struct.accuracy_matrix(correct_labels(comp,1),correct_labels(comp,2),set_idx,s_idx) = subj_acc(comp);
        end
    end
else
