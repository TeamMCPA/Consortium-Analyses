...
elseif isa(MCPA_pattern,'float'),
    ...
    data_file = input('Please enter path to the data file(s) (multiple files example:char(x.mat, y.mat, z.mat): ');
...
if length(data_file) == 1  %The case that input is either a single file or a connected file.
    ...
    if strcmp(ext,'.mcp'),
        ...
        nirs_data = extract_Homer_File(load(data_file{1}, '-mat'));
    end
    
elseif length(data_file) > 1 %The case that input need to be connected
    nirs_data = build_MCP(data_file,num2str(1:length(data_file)),'probe','s');
    
else 
    fprintf('There is something wrong wih the file input...');
    fprintf('return from wrong if clause...')
    return
end

%% Scan through the marksvector and note all unique event types (marks) if the input pattern is a double matrix or the field event_types does exist in the input MCPA_struct.

if isa(MCPA_pattern,'float')
...
for i = 1:num_subjs
    ...
    %Homer1 format
    if Homer_version(nirs_data) == 1
        %fprintf('Homer1... Under construction!!\n');
        oxy_timeser = nirs_data(incl_subjects(i)).hmr.data.dConc(:,incl_channels,1);
        marks_vec = nirs_data(incl_subjects(i)).otp.marksvector;
        
    %Homer2 format
    elseif Homer_version(nirs_data) == 2
        oxy_timeser = nirs_data(incl_subjects(i)).procResult.dc(:, 1, incl_channels);
        %switch the columns of 2 and 3 and the oxy_timeser's dimension will be (data length x channels x Hbo) just like dConc's dimension in Homer1
        oxy_timeser = permute(oxy_timeser, [1 3 2]);
        marks_vec = nirs_data(incl_subjects(i)).aux;
        
    else
        oxy_timeser = nirs_data(incl_subjects(i)).fNIRS_Data.Hb_data.Oxy(:,incl_channels);
        marks_vec = nirs_data(incl_subjects(i)).fNIRS_Data.Onsets_Matrix;
        marks_vec = marks_vec*[1:size(marks_vec,2)]';
    ...
    for currentmark = 1:length(totalmarks)
        ...
        %Homer2
        elseif Homer_version(nirs_data) == 2
            events(currentmark).marks = find(marks_vec(:, currentmark) == 1);
            events(currentmark).marks = events(currentmark).marks(1:2:end);          
        end
