...
%EXTRACT_VECS_1SUBJ  Takes epoched data from fNIRS time series (stored in
%.nirs files) and summarizes epoched data into multivariate pattern
%vectors. Additional exclusion of trials (beyond preprocessing) may be
%specified. Conditions are named by a condition name list, must be in the
%same order as the integer or binary stimulus flags in .nirs file.
%
...
if ~exist('exclude_by','var'),
    fprintf('No additional artifact exclusion for these data.\n');
    exclude_by = 'none';
else
    if ~(strcmp(exclude_by,'channel') || strcmp(exclude_by,'trial') || strcmp(exclude_by,'obsv') || strcmp(exclude_by,'none')),
        fprintf('ARTIFACT EXCLUSION METHOD NOT RECOGNIZED. Defaulting to "none".\n');
        exclude_by = 'none';
    end
end
