function allsubj_results = create_results_struct(within_subjects, summed_mcpa, parsed_input, all_sets, num_subj, num_sets, num_feature, num_cond, final_dimensions)
...
for cond_id = 1:num_cond % now create place holders for decoding accuracies 
    ...
    allsubj_results.train_accuracy(cond_id).condition = allsubj_results.conditions(cond_id);
    allsubj_results.train_accuracy(cond_id).subjXfeature = nan(num_subj,num_feature);
    allsubj_results.train_accuracy(cond_id).subsetXsubj = nan(num_sets,num_subj);
    if within_subjects
         allsubj_results.train_accuracy(cond_id).subjXsession = nan(num_subj,max_sessions);
    end
end

if within_subjects
    if isfield(parsed_input.Results.opts_struct, 'pairwise') && parsed_input.Results.opts_struct.pairwise == 1
        allsubj_results.accuracy_matrix_test = nan(length(allsubj_results.conditions),...
         ...
         allsubj_results.accuracy_matrix_train = nan(length(allsubj_results.conditions),...
            length(allsubj_results.conditions),...
            min(size(allsubj_results.subsets,1),...
            allsubj_results.max_sets),...
            size(summed_mcpa.patterns, ndims(summed_mcpa.patterns))-1,...
